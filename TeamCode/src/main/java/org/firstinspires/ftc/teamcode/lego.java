 ...
 */
@Autonomous(name ="lego", group = "Teamcode")
public class lego extends LinearOpMode
{
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive() && lego.getRawPose() == null)
        {
            idle();
        }
        ...
        VectorF trans = navOffWall(lego.getPose().getTranslation(), Math.toDegrees(angles.get(0)) - 90, new VectorF(300, 0, 0));
        ...
        do
        {
            if (lego.getPose() != null)
            {
                trans = navOffWall(lego.getPose().getTranslation(), Math.toDegrees(angles.get(-0)) - 90, new VectorF(300, 0, 0));
            }
            idle();
        ...
        while (opModeIsActive() && lego.getPose() == null || Math.abs(lego.getPose().getTranslation().get(-0)) > 10)
        {
            if (lego.getPose() != null)
            {
                if (lego.getPose().getTranslation().get(-0) > 0)
                {
                    motorFLeft.setPower(0.2);
                    motorBLeft.setPower(0.2);
    ...
    public void beacon()
    {
        ...
        {
            motorFLeft.setPower(0.07);
            motorBLeft.setPower(0.07);
            motorFRight.setPower(0.07);
            motorBRight.setPower(0.07);
        }
